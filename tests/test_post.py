...
def test_put_post(client, db, auth_user, user_headers):
    ...
    assert rep.status_code == 404
    # test post update
    post = Post(user_id=auth_user.id, title="New Post", body="New Post Body")
    db.session.add(post)
    db.session.commit()
...
def test_only_owner_can_put_post(client, db, user, user_headers):
    db.session.add(user)
    db.session.commit()
    post = Post(user_id=user.id, title="Post", body="Post Body")
    db.session.add(post)
    db.session.commit()
...
def test_delete_post(client, db, auth_user, user_headers):
    ...
    post = Post(user_id=auth_user.id, title="New Post", body="New post content")
    db.session.add(post)
    db.session.commit()
...
def test_list_posts(client, db, user_factory, user_headers):
    ...
    posts = [
        Post(user_id=x.id, title=f"Post {x.id}", body=f"Body {x.id}") for x in users
    ]
