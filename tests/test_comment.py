...
def test_get_post(client, db, comment, user_headers):
    # test 404 for missing post
    post_url = url_for("api.comment_by_id", post_id='100000')
    ...
    post_url = url_for('api.comment_by_id', post_id=comment.id)
    rep = client.get(post_url, headers=user_headers)
    assert rep.status_code == 200

    post_info = rep.get_json()["comment"]
    assert post_info["title"] is not None
    assert post_info["body"] is not None
